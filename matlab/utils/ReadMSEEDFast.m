...
function [trace] = DecodeSignal(encodedSignalMatrix, encodingFormat,isLittleEndian,BLOCK_RECORD_SIZE)
...
%             % --- decoding format: IEEE double precision floating point
        ...
        case {10,11,19}
            ...
            Q=signalMatrix(1:16:length(signalMatrix(:,1)),:);
...
%	D = [YEAR,DAY,HOUR,MINUTE,SECONDS]
    ...
    d = [Year,DayOfYear,Hours,Minutes,uint16(Seconds) + Seconds0001/1e4];

end

function [dateTimeStruct, dateTimeString, unixTimeStamp]= ConstructDateTime(dateTimeMatrix)
    ...
    dvec=datevec(datenum([double(dateTimeMatrix(1)),double(month),double(day),double(dateTimeMatrix(3)),double(dateTimeMatrix(4)),double(dateTimeMatrix(5))]));
    ...
    unixTimeStamp=FromMatlabTimeToUnixTime(datenum([dateTimeStruct.year,dateTimeStruct.month,dateTimeStruct.day,dateTimeStruct.hour,dateTimeStruct.minute,dateTimeStruct.second]));
end
